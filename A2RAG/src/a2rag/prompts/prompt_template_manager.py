...
class PromptTemplateManager:
    ...
    role_mapping: Dict[str, str] = field(
        ...
        metadata={"help": "Mapping from default roles in prompte template files to specific chat providers' defined roles."}
    )
    ...
    def _load_templates(self) -> None:
        ...
        for filename in os.listdir(self.templates_dir):
            if filename.endswith(".py") and filename != "__init__.py":
                ...
                try:
                    try:
                        module_name = f"a2rag.prompts.templates.{script_name}"
                        module = importlib.import_module(module_name)
                    except ModuleNotFoundError:
                        module_name = f".prompts.templates.{script_name}"
                        module = importlib.import_module(module_name, 'a2rag')
