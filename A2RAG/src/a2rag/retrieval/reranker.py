...
class DSPyFilter:
    def __init__(self, a2rag):
        """
        Initializes the object with the necessary configurations and templates for processing input and output messages.

        Parameters:
        a2rag : An object that provides the global configuration and the chat model required for inference.
        ...
        chat_infer_fn : A function reference for making inferences using the provided chat model.
        ...
        """
        dspy_file_path = a2rag.global_config.rerank_dspy_file_path
...
Respond with the corresponding output fields, starting with the field `[[ ## fact_after_filter ## ]]` containing ONLY a valid JSON object like {{"fact":[["s","p","o"]]}} and no extra text, and then ending with the marker for `[[ ## completed ## ]]`."""
        self.one_output_template = """[[ ## fact_after_filter ## ]]\n{fact_after_filter}\n\n[[ ## completed ## ]]"""
        self.message_template = self.make_template(dspy_file_path)
        self.chat_infer_fn = a2rag.chat_model_client.infer
        self.model_name = a2rag.global_config.chat_model
    ...
    def chat_call(self, question, fact_before_filter):
        # make prompt
        ...
        self.default_gen_kwargs['max_completion_tokens'] = 4096

        response = self.chat_infer_fn(
            messages=messages,
    ...
    def rerank(self,
        ...
        try:
            ...
            response = self.chat_call(query, json.dumps(fact_before_filter))
        ...
        if not generated_facts:
            return candidate_indices[:len_after_rerank], candidate_items[:len_after_rerank], {'fallback': 'parse_failed_or_empty'}
        ...
        if not sorted_candidate_indices:
            return candidate_indices[:len_after_rerank], candidate_items[:len_after_rerank], {'fallback': 'no_match'}
