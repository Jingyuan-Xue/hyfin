...
def reason_step(dataset, prompt_template_manager: PromptTemplateManager, query: str, passages: list, thoughts: list, chat_client: ChatGateway):
    """
    Given few-shot samples, query, previous retrieved passages, and previous thoughts, generate the next thought.
    :return: next thought
    """
    ...
    try:
        response_message, metadata = chat_client.infer(messages=messages)
        response_content = response_message[0]["content"]
    except Exception as e:
        logger.exception("An exception occurred while calling chat for QA!")
        return ''
